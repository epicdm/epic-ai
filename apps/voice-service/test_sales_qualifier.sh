...
# Sales Qualifier v1 - End-to-End Test Script
# Tests the complete DTMF-driven sales qualification flow

set -e

BASE_URL="${BASE_URL:-http://localhost:5000}"
SESSION_ID="test:sales:$(date +%s)"
...
echo ""
read -p "Press Enter to continue to Step 7 (Request Callback)..."
echo ""

# Step 7: Request Callback (DTMF 1)
echo "Step 7: Requesting Callback (DTMF 1)..."
curl -sS -X POST "$BASE_URL/telephony/session/continue" \
  -H 'Content-Type: application/json' \
  -d "{
    \"sessionId\": \"$SESSION_ID\",

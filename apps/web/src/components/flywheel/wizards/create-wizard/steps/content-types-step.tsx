...
export function ContentTypesStep({ data, updateData }: ContentTypesStepProps) {
  ...
  const toggleType = (typeId: ContentType) => {
    if (enabledTypes.includes(typeId)) {
      updateData({ enabledTypes: enabledTypes.filter((t) => t !== typeId) as ContentType[] });
    } else {
      updateData({ enabledTypes: [...enabledTypes, typeId] as ContentType[] });
    }
  };
